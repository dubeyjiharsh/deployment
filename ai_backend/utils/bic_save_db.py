...
def save_bic_response(user_name: str, session_id: str, document_id: str, update_data: dict):
    try:
        now = datetime.now()
        with connection.cursor() as cursor:
            # Check if submission exists
            cursor.execute("SELECT id FROM submissions WHERE session_id = %s AND doc_id = %s", (session_id, document_id))
            result = cursor.fetchone()
            ...
            if result:
                ...
                cursor.execute("""
                    INSERT INTO submissions (session_id, user_name, request_number, submitted_date, last_updated, status, doc_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (session_id, user_name, request_number, now, now, "in_progress", document_id))
                submission_id = cursor.lastrowid
    
            # Insert/update dynamic fields
            if update_data is not None or update_data != {} and isinstance(update_data, dict):
