import json
import ast
from utils.db_utils import get_connection, create_table_if_not_exists
from utils.logutils import log_context
 
def ui_2_db(user_name, session_id, document_id, session_data, timestamp):
    if not user_name or not document_id or not session_id or session_data is None:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT session_data FROM user_sessions WHERE id=%s AND doc_id=%s", (item_id, document_id))
            result = cursor.fetchone()
            
            if result:
                cursor.execute("UPDATE user_sessions SET session_data = %s WHERE id = %s AND doc_id=%s", (session_data, item_id, document_id))

            else:
                cursor.execute("""
                    INSERT INTO user_sessions (id, user_name, session_id, session_data, timestamp, doc_id)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE session_data=%s, timestamp=%s
                """, (item_id, user_name, session_id, session_data_str, timestamp, document_id, session_data_str, timestamp))
...
def db_2_ui(user_name, session_id, document_id):
    if not user_name or not session_id or not document_id:
        log_context('error', user_name, session_id, "Missing required parameters")
...
def delete_session_item(user_name, session_id, document_id):
    if not user_name or not session_id or not document_id:
        log_context('error', user_name, session_id, "Missing required parameters")
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("DELETE FROM user_sessions WHERE id=%s AND doc_id=%s", (item_id, document_id))
