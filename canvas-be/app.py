...
def run_thread_with_assistant(client: AzureOpenAI, thread_id: str, assistant_id: str, user_message: str, file_ids: List[str] = None) -> dict:
    """
    ...
    Args:
        ...
        file_ids (List[str], optional): The list of file_ids to attach. Defaults to None.
    Returns:
        dict: The completed run information.
    """
    attachments = [{"file_id": file_id, "tools": [{"type": "file_search"}]} for file_id in file_ids] if file_ids else []
...
class FileUploadResponse(BaseModel):
    file_ids: List[str]  # Change to a list of file_ids

# FastAPI endpoint for uploading files (now supports multiple files)
@app.post("/upload_files/", response_model=FileUploadResponse)
async def upload_files_endpoint(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Upload multiple files and return their file_ids.
    Args:
        files (List[UploadFile]): The list of files to be uploaded.
    Returns:
        JSONResponse: A JSON response containing the file_ids.
    """
    try:
        file_ids = []
...
async def ask_assistant(request: QueryRequest, canvas_id: str, file_ids: Optional[List[str]] = None)-> JSONResponse:
    """
    ...
    # If the file_ids were not provided in the request, use the first uploaded file_id
    if not file_ids:
        if not session_files:
            raise HTTPException(status_code=400, detail="No files uploaded yet. Please upload files first.")
        file_ids = list(session_files.values())
