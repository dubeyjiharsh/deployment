...
const OPENAI_MODEL = "gpt-4o"; // GPT-4 Optimized for chat and complex tasks
 ...
 */
function handleOpenAIError(error: unknown, operation: string): never {
  ...
  if (error instanceof Error) {
    ...
    if (errorMessage.includes('rate limit') || errorMessage.includes('too many requests')) {
      ...
      throw new Error("OpenAI API key configuration issue. Please check your OpenAI API key in settings.");
...
/**
 * Vercel AI SDK provider implementation for OpenAI
 * NOTE: OpenAI provider only supports text generation, NOT embeddings (RAG uses OpenAI embeddings separately)
 */
export class OpenAIProvider implements ILlmProvider {
  ...
  /**
   * Get the OpenAI model instance with optional caching
   */
  private getModel() {
    const baseModel = this.openai(OPENAI_MODEL);
...
Use these as primary sources when refining this field.`
      ...
      handleOpenAIError(error, "Field refinement");
    }
  }
  ...
  async *streamCanvasGeneration(
    ...
    try {
      console.log("🤖 Using Vercel AI SDK with OpenAI for streaming generation");
...
- DO NOT use generic names - use the EXACT filename from the document header above`
      ...
      handleOpenAIError(error, "Canvas generation streaming");
    }
  }
...
Use these documents as primary sources when expanding the canvas fields.`
      ...
      console.log("📝 Vercel AI SDK (OpenAI) response for expansion:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Canvas expansion");
    }
  }
  ...
  async generateStories(canvas: BusinessCanvas): Promise<import("@/stores/canvas-store").Story[]> {
    try {
      ...
      console.log("📝 Vercel AI SDK (OpenAI) response for stories:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Story generation");
    }
  }
  ...
  async generateExecutionPlan(
    ...
    try {
      ...
      console.log("📅 Vercel AI SDK (OpenAI) response for execution plan:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Execution plan generation");
    }
  }
  ...
  > {
    try {
      ...
      console.log("🔍 Vercel AI SDK (OpenAI) response for conflicts:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Conflict detection");
    }
  }
  ...
  async generateBenchmarks(
    ...
    try {
      ...
      console.log("📊 Vercel AI SDK (OpenAI) response for benchmarks:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Benchmark generation");
    }
  }
  ...
  async generateEpicsFromOKRs(
    ...
    try {
      ...
      console.log("🎯 Vercel AI SDK (OpenAI) response for epics:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Epic generation");
    }
  }
  ...
  async generateEpicsFromBusinessRequirement(
    ...
    try {
      ...
      console.log("🎯 Vercel AI SDK (OpenAI) response for epics from business requirement:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Epic generation from business requirement");
    }
  }
  ...
  async generateFeaturesFromEpics(
    ...
    try {
      ...
      console.log("🎯 Vercel AI SDK (OpenAI) response for features:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "Feature generation");
    }
  }
  ...
  async generateUserStoriesFromFeatures(
    ...
    try {
      ...
      console.log("🎯 Vercel AI SDK (OpenAI) response for user stories:", text.substring(0, 200));
      ...
      handleOpenAIError(error, "User story generation");
    }
  }
